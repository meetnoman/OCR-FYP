...
public class TextTranslation extends AppCompatActivity {
    ...
    private void translateText(final String text, final String lang) {
        ...
        int inputLanguageCode=0;
        if (LanguageRemindHelper.getInstance().getInputLanguage().equalsIgnoreCase("Detect Language")){
            TextAugmenter obj=new TextAugmenter();
            for (String a:recognizedLanguages) {
                if (obj.getAbbrevation(a)!=null){
                    inputLanguageCode = FirebaseTranslateLanguage.languageForLanguageCode(a);
                }
            }
            ...
            inputLanguageCode = FirebaseTranslateLanguage.languageForLanguageCode(LanguageRemindHelper.getInstance().getInputLanguage());
        }
